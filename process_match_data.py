...
def match_data_to_dict(match_data):
    ...
    matchup_parts = [
        matchup_part.strip()
        for matchup_part in match_data[0].split()
        if matchup_part.strip() != ""
    ]
    ...
    date_entered_parts = [
        line_part.strip()
        for line_part in match_data[1].split()
        if line_part.strip() != ""
    ]

    if (
        date_entered_parts[5][:2] == "0:"
    ):  # handle 12th-hour bug (stored as "0:" instead of "12:")
        date_entered_parts[5] = "12:{:s}".format(date_entered_parts[3][2:])

    date_entered_str = "{0:s}-{1:s}{2:s} {3:s} {4:s}".format(
        date_entered_parts[1],
        date_entered_parts[2],
        date_entered_parts[3],
        date_entered_parts[5],
        date_entered_parts[6],
    )
    match_dict["date_entered"] = datetime.strptime(
        date_entered_str, "%B-%d,%Y %I:%M %p"
    )
    ...
    for round_data in match_data[2:]:
        ...
        round_parts = [
            round_part.strip()
            for round_part in re.split(",", round_data)
            if round_part.strip() != ""
        ]

        match_dict["p" + str(playerNum) + "_date_played"] = datetime.strptime(
            round_parts[1], "%b-%d-%Y"
        ).date()
        ...
        for hole_num in range(1, 10):
            match_dict["p" + str(playerNum) + "_h" + str(hole_num) + "_score"] = int(
                round_parts[3 + hole_num]
            )
        ...
        match_dict["p" + str(playerNum) + "_score_differential"] = float(
            round_parts[16]
        )
...
if __name__ == "__main__":
    ...
    for data_dir in data_dirs:
        ...
        scores_files = [
            f for f in os.listdir(f"data/{data_dir}") if f[0:7] == "scores."
        ]
        ...
        for scores_file in scores_files:
            ...
            match_data_list = parse_match_data_from_file(
                f"data/{data_dir}/{scores_file}"
            )
