        ...
        for line in fp:
            ...
            if line[:4].lower() == "team":
                ...
                if len(line_parts) != 10:
                    raise ValueError(
                        "Unable to parse roster player line: {:s}".format(line)
                    )
...
if __name__ == "__main__":
    ...
    for data_dir in data_dirs:
        ...
        roster_files = [
            f for f in os.listdir(f"data/{data_dir}") if f[0:7] == "roster."
        ]
        for roster_file in roster_files:
            ...
            roster_dict_list = []
            try:
                roster_dict_list = parse_roster_data_from_file(
                    f"data/{data_dir}/{roster_file}"
                )
            except ValueError:
            ...
            if len(roster_dict_list) == 0:
                ...
                for roster_dict in roster_dict_list:
                    csv_data += "\n" + ",".join(
                        [str(v) for k, v in roster_dict.items()]
                    )
