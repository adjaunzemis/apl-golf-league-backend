            ...
            if (len(line) > 4) and (line[0] != "#"):
                if line[:6].lower() == "course":
                    if line[:17].lower() == "course_parameters":
                        ...
                        front_course_matches = [
                            course
                            for course in courses
                            if course["abbreviation"] == front_name
                        ]
                        if len(front_course_matches) != 1:
                            raise ValueError(
                                "Found {:d} course matches for front id={:s}".format(
                                    len(front_course_matches), front_name
                                )
                            )
                        ...
                        back_course_matches = [
                            course
                            for course in courses
                            if course["abbreviation"] == back_name
                        ]
                        if len(back_course_matches) != 1:
                            raise ValueError(
                                "Found {:d} course matches for back id={:s}".format(
                                    len(back_course_matches), back_name
                                )
                            )
                        ...
                        if len(line_elements) == 9:
                            ...
                            front_course["ratings"] = [
                                np.around(float(rating) * front_par / total_par, 2)
                                for rating in line_elements[6:9]
                            ]
                            front_course["slopes"] = [
                                int(slope) for slope in line_elements[3:6]
                            ]

                            back_course["tee_names"] = ["Middle", "Senior", "Forward"]
                            back_course["ratings"] = [
                                np.around(float(rating) * back_par / total_par, 2)
                                for rating in line_elements[6:9]
                            ]
                            back_course["slopes"] = [
                                int(slope) for slope in line_elements[3:6]
                            ...
                            front_course["ratings"] = [
                                np.around(float(rating) * front_par / total_par, 2)
                                for rating in line_elements[4:11:2]
                            ]
                            front_course["slopes"] = [
                                int(slope) for slope in line_elements[3:10:2]
                            ]

                            back_course["tee_names"] = [
                                "Middle",
                                "Senior",
                                "Forward",
                                "Super-Senior",
...
if __name__ == "__main__":
    ...
    for data_dir in data_dirs:
        ...
        course_files = [
            f for f in os.listdir(f"data/{data_dir}") if f == "apl.courses.data"
        ]

        for course_file in course_files:
            print(f"Processing {data_year} courses file: {course_file}")

            course_data_list = parse_course_data_from_file(
                f"data/{data_dir}/{course_file}"
            )
