        ...
        for line in fp:
            ...
            if (len(line) > 0) and (
                ...
                if line_parts[0].lower() == "league_signup_date":
                    flight_dates["signup_start_date"] = datetime.strptime(
                        " ".join(line_parts[1:]), "%B %d %Y"
                    )
                elif line_parts[0].lower() == "end_league_signup_date":
                    flight_dates["signup_stop_date"] = datetime.strptime(
                        " ".join(line_parts[1:]), "%B %d %Y"
                    )
                elif line_parts[0].lower() == "league_start_date":
                    flight_dates["start_date"] = datetime.strptime(
                        " ".join(line_parts[1:]), "%B %d %Y"
                    )
    return flight_dates
...
if __name__ == "__main__":
    ...
    for data_dir in data_dirs:
        ...
        flight_files = [
            f for f in os.listdir(f"data/{data_dir}") if f == "apl.flights.data"
        ]
        for flight_file in flight_files:
            print(f"Processing {data_year} flights file: {flight_file}")
            flight_dict_list = []
            try:
                flight_dict_list = parse_flight_data_from_file(
                    f"data/{data_dir}/{flight_file}"
                )
            except ValueError:
            ...
            try:
                flight_dates = parse_season_parameters(
                    f"data/{data_dir}/season_parameters.data"
                )
            except ValueError:
            ...
            if len(flight_dict_list) == 0:
                ...
                for flight_dict in flight_dict_list:
                    flight_schedule = parse_flight_schedule(
                ...
                for flight_dict in flight_dict_list:
                    csv_data += "\n" + ",".join(
                        [str(v) for k, v in flight_dict.items()]
                    )
