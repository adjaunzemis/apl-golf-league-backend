...
def get_rounds_in_scoring_record(session: Session, golfer_id: int, min_date: dt_date, max_date: dt_date, limit: int = 20, use_legacy_handicapping: bool = False) -> List[RoundSummary]:
    ...
    if len(round_summaries) < 2: # include qualifying scores
        qualifying_scores_db = session.exec(select(QualifyingScore).where(QualifyingScore.golfer_id == golfer_id).where(QualifyingScore.year == max_date.year)).all()
...
if __name__ == "__main__":
    ...
    with Session(engine) as session:
        ...
        update_golfer_handicaps(session=session, old_max_date=datetime(2022, 8, 29), new_max_date=datetime(2022, 9, 5)) # TODO: un-hardcode dates
    print('Done!')
