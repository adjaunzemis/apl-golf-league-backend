...
class WorldHandicapSystem(HandicapSystem):
    ...
    def compute_hole_adjusted_gross_score(
        ...
        return min(
            score,
            self.compute_hole_maximum_score(
                par, stroke_index, course_handicap=course_handicap
            ),
        )
    ...
    def compute_hole_maximum_score(
        ...
        if course_handicap is None:  # handicap not established
            return par + 5
        return min(
            par + 2 + self.compute_hole_handicap_strokes(stroke_index, course_handicap),
            par + 5,
        )
    ...
    def compute_score_differential(
        ...
        slope: int,
        score: int,
        playing_conditions_correction: float = 0.0,
    ):
    ...
    def compute_handicap_index(self, record: List[float]) -> float:
        ...
        return np.round(
            min(handicap_index, self.maximum_handicap_index), 1
