...
def add_scheduled_matches(*, session: Session, flight_db: Flight):
    ...
    if (not flight_team_links_db) or (len(flight_team_links_db) == 0):
        raise ValueError("Unable to find teams in flight: " + flight_db.name + " (" + flight_db.year + ")")
    ...
    if (flight_db.weeks == 18) and (len(team_ids) == 5):
        ...
        raise ValueError(f"No pre-defined {flight_db.weeks}-week matchup matrix for {len(team_ids)} teams")
    ...
    for week_idx in range(len(matchup_matrix)):
        ...
        for team_idx in range(len(team_ids)):
            ...
            if matchup_matrix[week_idx][team_idx]: # check for byes
                ...
                match_db = session.exec(select(Match).where(Match.flight_id == flight_db.id).where(Match.week == week).where(((Match.home_team_id == team_id) & (Match.away_team_id == opponent_team_id)) | (Match.home_team_id == opponent_team_id) & (Match.away_team_id == team_id))).one_or_none()
                if not match_db:
                    print(f"Adding match: week={week}, home_team_id={team_id}, away_team_id={opponent_team_id}")
                    match_db = Match(flight_id=flight_db.id, week=week, home_team_id=team_id, away_team_id=opponent_team_id)
