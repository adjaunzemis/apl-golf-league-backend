...
def get_courses_by_id(session: Session, course_ids: list[int]) -> list[Course]:
    """Get courses from database with given identifiers.

    Parameters
    ----------
    session (`Session`): Database session.
    course_ids (list[int]): Course identifiers to query.

    Returns
    -------
    list[`Course`]: Courses from database.
    """
    return list(session.exec(select(Course).where(Course.id.in_(course_ids))).all())


def get_tracks_by_id(session: Session, track_ids: list[int]) -> list[Track]:
    """Get tracks from database with given identifiers.

    Parameters
    ----------
    session (`Session`): Database session.
    track_ids (list[int]): Track identifiers to query.

    Returns
    -------
    list[`Track`]: Tracks from database.
    """
    return list(session.exec(select(Track).where(Track.id.in_(track_ids))).all())


def get_tees_by_id(session: Session, tee_ids: list[int]) -> list[Tee]:
    """Get tees from database with given identifiers.

    Parameters
    ----------
    session (`Session`): Database session.
    tee_ids (list[int]): Tee identifiers to query.

    Returns
    -------
    list[`Tee`]: Tees from database.
    """
    return list(session.exec(select(Tee).where(Tee.id.in_(tee_ids))).all())


def get_holes_by_id(session: Session, hole_ids: list[int]) -> list[Hole]:
    """Get holes from database with given identifiers.

    Parameters
    ----------
    session (`Session`): Database session.
    hole_ids (list[int]): Hole identifiers to query.

    Returns
    -------
    list[`Hole`]: Holes from database.
    """
    return list(session.exec(select(Hole).where(Hole.id.in_(hole_ids))).all())
...
def get_course_data_by_tee_id(
    ...
    """
    tee_db = next(iter(get_tees_by_id(session=session, tee_ids=[tee_id])), None)
    if tee_db is None:
        raise ValueError(f"Unable to find tee with id={tee_id}")

    track_db = next(
        iter(get_tracks_by_id(session=session, track_ids=[tee_db.track_id])), None
    )
    if track_db is None:
        raise ValueError(f"Unable to find track with id={tee_db.track_id}")

    course_db = next(
        iter(get_courses_by_id(session=session, course_ids=[track_db.course_id])), None
    )
