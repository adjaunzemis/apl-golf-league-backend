...
async def read_courses(*, session: Session = Depends(get_sql_db_session), offset: int = Query(default=0, ge=0), limit: int = Query(default=100, le=100)):
...
async def create_course(*, session: Session = Depends(get_sql_db_session), current_user: User = Depends(get_current_active_user), course: CourseCreate):
...
async def read_course(*, session: Session = Depends(get_sql_db_session), course_id: int):
...
async def update_course(*, session: Session = Depends(get_sql_db_session), current_user: User = Depends(get_current_active_user), course_id: int, course: CourseUpdate):
    course_db = session.get(Course, course_id)
    if not course_db:
        raise HTTPException(status_code=404, detail="Course not found")
...
async def delete_course(*, session: Session = Depends(get_sql_db_session), current_user: User = Depends(get_current_active_user), course_id: int):
    course_db = session.get(Course, course_id)
    if not course_db:
        raise HTTPException(status_code=404, detail="Course not found")
...
async def read_tracks(*, session: Session = Depends(get_sql_db_session), offset: int = Query(default=0, ge=0), limit: int = Query(default=100, le=100)):
...
async def read_track(*, session: Session = Depends(get_sql_db_session), track_id: int):
...
async def read_tees(*, session: Session = Depends(get_sql_db_session), offset: int = Query(default=0, ge=0), limit: int = Query(default=100, le=100)):
...
async def read_holes(*, session: Session = Depends(get_sql_db_session), offset: int = Query(default=0, ge=0), limit: int = Query(default=100, le=100)):
    return session.exec(select(Hole).offset(offset).limit(limit)).all()

@router.get("/holes/{hole_id}", response_model=HoleRead)
async def read_hole(*, session: Session = Depends(get_sql_db_session), hole_id: int):
