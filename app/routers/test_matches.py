...
@pytest.mark.parametrize("match_id, round_id", [(None, 2), (1, None)])
def test_create_match_round_link_incomplete(
    client: TestClient, match_id: int, round_id: int
):
    # Missing required fields
    response = client.post(f"/matches/{match_id}/rounds/{round_id}")
    assert response.status_code == 422
...
@pytest.mark.parametrize(
    "match_id, round_id", [({"key": "value"}, 2), (1, {"key": "value"})]
)
def test_create_match_round_link_invalid(
    client: TestClient, match_id: int, round_id: int
):
    # Invalid input data types
    response = client.post(f"/matches/{match_id}/rounds/{round_id}")
