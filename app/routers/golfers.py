...
from ..models.query_helpers import GolferData, GolferDataWithCount, GolferTeamData, get_golfers, get_golfer_team_data
...
async def read_golfers(*, session: Session = Depends(get_sql_db_session), offset: int = Query(default=0, ge=0), limit: int = Query(default=100, le=100)):
...
async def create_golfer(*, session: Session = Depends(get_sql_db_session), golfer: GolferCreate):
...
async def read_golfer(*, session: Session = Depends(get_sql_db_session), golfer_id: int, min_date: date = Query(default=date(date.today().year - 2, 1, 1)), max_date: date = Query(default=date.today() + timedelta(days=1))):
...
async def update_golfer(*, session: Session = Depends(get_sql_db_session), current_user: User = Depends(get_current_active_user), golfer_id: int, golfer: GolferUpdate):
...
async def delete_golfer(*, session: Session = Depends(get_sql_db_session), current_user: User = Depends(get_current_active_user), golfer_id: int):
    golfer_db = session.get(Golfer, golfer_id)
    if not golfer_db:
        raise HTTPException(status_code=404, detail="Golfer not found")
...
async def read_golfer_team_data(*, session: Session = Depends(get_sql_db_session), golfer_id: int, year: int = Query(default=None)):
