...
class HoleResultBase(APLGLBaseModel):
...
class HoleResultUpdate(APLGLBaseModel):
...
class HoleResultData(APLGLBaseModel):
    hole_result_id: int
    round_id: int
    hole_id: int
    number: int
...
class HoleResultValidationRequest(APLGLBaseModel):
    number: int
    par: int
    stroke_index: int
    gross_score: int
