...
def get_flights(session: Session, flight_ids: list[int]) -> list[FlightData]:
    ...
    flights_db = session.exec(
        select(Flight).where(Flight.id.in_(flight_ids)).order_by(Flight.year)
    ).all()
    courses_db = session.exec(
        select(Course).where(
    ...
    for flight_db in flights_db:
        flight_courses_db.append(
            [
                ...
                if course_db.id == flight_db.course_id
            ][0]
            if flight_db.course_id
            else None
        )
    return [
        FlightData(
...
def get_tournaments(
    ...
    """
    tournament_query_data = session.exec(
        select(Tournament, Course)
        .join(Course)
...
def get_divisions_in_flights(
    ...
    division_query_data = session.exec(
        ...
        .where(FlightDivisionLink.flight_id.in_(flight_ids))
    )
    return [
        DivisionData(
            id=division.id,
...
def get_divisions_in_tournaments(
    ...
    division_query_data = session.exec(
        ...
        .join(secondary_tee, onclause=Division.secondary_tee_id == secondary_tee.id)
        .join(secondary_track, onclause=secondary_tee.track_id == secondary_track.id)
        .where(TournamentDivisionLink.tournament_id.in_(tournament_ids))
    )
    return [
        DivisionData(
            id=division.id,
...
def get_teams_in_flights(
    ...
    team_query_data = session.exec(
        select(Team.id, Team.name, Flight)
    ...
    team_golfers = get_flight_team_golfers_for_teams(
        session=session,
        team_ids=(team_id for team_id, team_name, flight in team_query_data),
    )
    return [
        FlightTeamReadWithGolfers(
            id=team_id,
            flight_id=flight.id,
            name=team_name,
            golfers=[golfer for golfer in team_golfers if golfer.team_id == team_id],
        )
        for team_id, team_name, flight in team_query_data
    ]
...
def get_teams_in_tournaments(
    ...
    """
    team_query_data = session.exec(
        select(Team.id, Team.name, Tournament)
    ...
    team_golfers = get_tournament_team_golfers_for_teams(
        session=session,
        team_ids=(team_id for team_id, team_name, tournament in team_query_data),
    )
    return [
        TournamentTeamData(
            id=team_id,
        ...
        for team_id, team_name, tournament in team_query_data
    ]
...
def get_flight_team_golfers_for_teams(
    ...
    query_data = session.exec(
        ...
        .where(TeamGolferLink.team_id.in_(team_ids))
    )
    return [
        GolferTeamData(
...
def get_tournament_team_golfers_for_teams(
    ...
    query_data = session.exec(
        ...
        .join(
            Tournament, onclause=TournamentDivisionLink.tournament_id == Tournament.id
        )
        .where(TeamGolferLink.team_id.in_(team_ids))
    )
    return [
...
def get_matches(session: Session, match_ids: list[int]) -> list[MatchData]:
    ...
    match_query_data = session.exec(
        select(Match, Flight, home_team, away_team)
        .join(Flight, onclause=Match.flight_id == Flight.id)
        .join(home_team, onclause=Match.home_team_id == home_team.id)
        .join(away_team, onclause=Match.away_team_id == away_team.id)
...
def get_rounds_for_matches(
    ...
    round_ids = session.exec(
        select(Round.id)
        .join(MatchRoundLink, onclause=MatchRoundLink.round_id == Round.id)
        .where(MatchRoundLink.match_id.in_(match_ids))
    ).all()
    ...
    for round in rounds:
        if round.golfer_id == golfer_id:
            ...
            stats.avg_gross_score += (
                round.gross_score - stats.avg_gross_score
            ) / stats.num_rounds
            stats.avg_net_score += (
                round.net_score - stats.avg_net_score
            ) / stats.num_rounds
...
def get_rounds_in_scoring_record(
    ...
    if len(round_summaries) < 2:  # include qualifying scores
        qualifying_scores_db = session.exec(
            ...
            .where(QualifyingScore.date_played >= min_date)
            .where(QualifyingScore.date_played <= max_date)
        ).all()
...
from app.database.golfers import (
    get_golfer_team_data as get_golfer_team_data,
)
from app.database.golfers import (
    get_golfer_year_joined as get_golfer_year_joined,
)
from app.database.golfers import (
    get_golfers as get_golfers,
)
